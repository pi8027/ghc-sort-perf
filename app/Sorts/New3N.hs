module Sorts.New3N (sort, sortBy) where
...
actualSort gt ns
  ...
  where
    sequences (a:b:c:xs) =
      if a `gt` b then
        if a `gt` c then
          if b `gt` c then descending c [b,a] xs else [b,c,a] : sequences xs
        else [b,a,c] : sequences xs
      else
        if b `gt` c then
          if a `gt` c then [c,a,b] : sequences xs else [a,c,b] : sequences xs
        else ascending c (\ys -> a:b:ys) xs
    sequences [a,b]
      | a `gt` b = [[b,a]]
      | otherwise = [[a,b]]
    sequences xs = [xs]
