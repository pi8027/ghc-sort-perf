module Sorts.New4WMParallel (sort, sortBy) where
import Control.Concurrent (newMVar, newEmptyMVar, putMVar, forkIO)
import GHC.MVar (MVar(MVar))

sort :: Ord a => [a] -> IO [a]
sort = sortBy compare

sortBy :: (a -> a -> Ordering) -> [a] -> IO [a]
sortBy cmp ns
  | []        <- ns = pure []
  | [a]       <- ns = pure [a]
  | [a,b]     <- ns = pure $ merge [a] [b]
  | [a,b,c]   <- ns = pure $ merge3 [a] [b] [c]
  | [a,b,c,d] <- ns = pure $ merge4 [a] [b] [c] [d]
